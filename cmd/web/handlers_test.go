...
func TestPing(t *testing.T) {
	// t.Parallel() // run test concurrently alongside other tests

	// rr := httptest.NewRecorder() // init httptest.ResponseRecorder
...
func TestSignupUser(t *testing.T) {
	...
	tests := []struct {
		...
		{"Invalid email (incomplete domain)", "Bob", "bob@example.", "validPa$$word", csrfToken, http.StatusOK, []byte("Invalid Field")},
		{"Invalid email (missing @)", "Bob", "bobexample.com", "validPa$$word", csrfToken, http.StatusOK, []byte("Invalid Field")},
		{"Invalid email (missing local part)", "Bob", "@example.com", "validPa$$word", csrfToken, http.StatusOK, []byte("Invalid Field")},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// t.Logf("testing %q for want-code %d and want-body %q", tt.name, tt.wantCode, tt.wantBody)
			...
			if !bytes.Contains(body, tt.wantBody) {
				t.Errorf("want body to contain %q; got %s", tt.wantBody, body)
			}
